...
int disparity(image2d_t left, image2d_t right, int2 pos, int d)
{
 ...
 int color_similarity_r = abs_diff((int)(1000 * left_pixel.x), (int)(1000 * right_pixel.x));
 int color_similarity_g = abs_diff((int)(1000 * left_pixel.y), (int)(1000 * right_pixel.y));
 int color_similarity_b = abs_diff((int)(1000 * left_pixel.z), (int)(1000 * right_pixel.z));
 ...
 )
{
    ...
    for (int d = 0; d >= -60; d--)
    {
     ...
     for (int i = v_minus; i <= v_plus; i++)
     {
      ...
      {
       disp_sum = disp_sum + disparity(input_l, input_r, pos + (int2)(j, i), d);
      }
     }
     result = (float)(disp_sum) / (float)(pix_num);

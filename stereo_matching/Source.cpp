...
int main()
{
 ...
 Image img;
 lodepng::decode(img.pixel,img.width,img.height, "img/view1.png");


 static const cl_image_format format = { CL_RGBA, CL_UNORM_INT8 };
 cl_mem inputImage = clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &format,  img.width, img.height, 0,  // This is a bug in the spec
  const_cast<unsigned char*> (img.pixel.data()),  &error);

 cl_mem outputImage = clCreateImage2D(context, CL_MEM_WRITE_ONLY, &format,  img.width, img.height, 0,  nullptr, &error);
 ...
 std::size_t size[3] = { img.width, img.height, 1 };
 ...
 lodepng::encode("img/niewiem.png", result.pixel , result.width, result.height);
